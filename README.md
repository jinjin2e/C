...
> 변수의 이름 앞에 사용해서 변수의 주소값을 반환합니다. (포인터 변수를 선언 할 때 값 부분, 또는 인자값을 포인터로 받는 함수에서 사용)    
#### 2. *(참조 연산자)
> 포인터의 이름이나 주소 앞에 사용함(포인터 변수를 선언 할 떄 변수명 앞쪽에 사용, 또는 메모리 주소에 접근해 값을 이용할 때 사용)    
...
***
## if
중첩 if 혹은 &&를 사용해서 여러 조건의 if를 사용할 때 조건이 어려운 if를 앞쪽에 두어 mcu 연산 소비를 줄인다.  
if( A && B )  
A가 0이라면 B를 확인하지 않음 -> A가 1일때만 동작하여야 할 함수를 B 위치에 위치시키고 그 함수의 return 1 일 때 동작하는 if를 만들 수 있음   

## macro 

매크로를 만들어 사용하면 같은 기능을 하는 함수를 호출하는 것 보다 빠르고 효율적임.  
#define SUM(X, Y) ((X)+(Y))      

void SUM(uint8_t x, uint8_t y){
  return x+y;
}
